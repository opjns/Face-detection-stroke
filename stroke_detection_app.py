...
class SimpleCNN(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.fc1 = nn.Linear(197136, num_classes)  # trained size
        self.num_classes = num_classes
        self._flatten_dim = None  # auto-detect for new inputs

    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x)))
        x = torch.flatten(x, 1)

        # If new input size, re-init fc1 dynamically
        if self._flatten_dim is None:
            self._flatten_dim = x.size(1)
        if x.size(1) != self._flatten_dim:
            # rebuild the layer if different input size (for flexibility)
