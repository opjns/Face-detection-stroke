...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        # Avoid hardcoding feature map size
        self.fc1 = nn.Linear(16 * 112 * 112, 2)

    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x)))
        x = torch.flatten(x, 1)  # ✅ auto-flatten instead of hardcoded shape
        x = self.fc1(x)
        return x
...
st.write("Upload a **face image or short video** to predict stroke likelihood based on facial features.")
...
if option == "Image":
    ...
    if image_file and st.button("Predict Stroke"):
        ...
        label = "⚠️ Stroke Detected" if prediction == 1 else "✅ No Stroke Detected"
        st.subheader(label)
...
if option == "Video":
    ...
    if video_file and st.button("Analyze Video"):
        ...
        if predictions:
            ...
            label = "⚠️ Stroke Detected" if stroke_ratio > 0.5 else "✅ No Stroke Detected"
            st.subheader(label)
